...
def load_kg(kg_file, inverse, target_relation=None):
    kg_ori = []
    kg_inv = []
...
if __name__ == '__main__':
    # total_size = len(test_kg)
    target_file = 'relations.dict'
    ...
    total_kg = load_kg('{}/test.txt'.format(data_dir), False)
    ...
    with open('{}/{}'.format(data_dir, target_file), mode='r') as fd:
        for line in tqdm(fd.readlines()):
            if not line: continue
            try:
                target_relation = line.strip().split('\t')[-1]
                target_relation = target_relation.replace('/', '|')
                option = Option('{}/{}-{}-{}/'.format(exp, exp_name, target_relation, 'ori'))
                test_kg = load_kg('{}/test.txt'.format(option.data_dir), False, target_relation)
                valid_kg = load_kg('{}/valid.txt'.format(option.data_dir), False, target_relation)
