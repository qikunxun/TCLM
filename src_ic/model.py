...
class TCLMModel(nn.Module):
    def __init__(self, n, T, L, N, tau_1=10, tau_2=0.2, use_gpu=False, dropout=0.1):
    ...
    def forward(self, input_x, e2triple, triple2e, triple2r, flag, is_training=False):
        ...
        for t in range(self.T):
            ...
            hidden = self.activation(torch.sparse.mm(hidden, torch.permute(h_probs, (1, 0)))) # [N, L]
            hidden = hidden * (1 - beta.unsqueeze(dim=0)) + torch.ones_like(hidden) * beta.unsqueeze(dim=0)
            ...
            hidden_x = self.activation(torch.mm(hidden_x, torch.permute(h_x_probs, (1, 0))))  # [b, L]
            hidden_x = hidden_x * (1 - beta_x.unsqueeze(dim=0)) + \
                       torch.ones_like(hidden_x) * beta_x.unsqueeze(dim=0)    # [b, L]
