...
from main_type import create_graph, get_type_matrix
...
def load_kg_form_pkl(file_path, target_relation):
    ...
    with open(file_path + 'type2id_{}.pkl'.format(target_relation.replace('/', '|')), mode='rb') as fd:
        type2id = pkl.load(fd)
    return kg, entity2id, relation2id, triple2id, entity_type, type2id
...
def load_kg(kg_file, id2entity, id2relation):
    kg = []
...
def evaluate(id2entity, entity2id, id2relation, relation2id, triple2id, entity_type, type2id, train_kg, eval_kg,
             option, model_save_path):
    ...
    model = TCLMModel(len(relation2id), option.step, option.length,
                           len(entity2id), len(type2id), option.tau_1, option.tau_2, option.use_gpu)
    ...
    entity_matrix = get_type_matrix(entity_type, entity2id, type2id)
    if option.use_gpu: entity_matrix = entity_matrix.cuda()
    id2head = {}
    head2id = {}
    ...
    for i in tqdm(range(batch_num)):
        ...
        state = model(input_x, entity_matrix, e2triple, triple2e, triple2r, inverse, is_training=False)
...
if __name__ == '__main__':
    ...
    train_kg, entity2id, relation2id, triple2id, entity_type, type2id = load_kg_form_pkl('{}/'.format(option.exp_dir),
                                                                   option.target_relation.replace('/', '|'))
    id2entity = reverse(entity2id)
    id2relation = reverse(relation2id)
    eval_kg = load_kg('{}/test/Fact.txt'.format(option.data_dir), id2entity, id2relation)
    evaluate(id2entity, entity2id, id2relation, relation2id, triple2id, entity_type, type2id, train_kg, eval_kg, option,
